...
export default function ProjectDetail() {
  ...
  useEffect(() => {
    track("view_project", { slug: p.slug, title: p.title });
  }, [p.slug, p.title]);
        ...
        >
          <Card className="p-6 sm:p-8">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <MiniCard title="Outcome / impact">
                {(p.impact ?? []).length ? (
                  <ul className="list-disc space-y-1 pl-5">
                    {(p.impact ?? []).slice(0, 2).map((x) => <li key={x}>{x}</li>)}
                  </ul>
                ) : (
                  <span className="text-slate-600">Add 1–2 quantified bullets here.</span>
                )}
              </MiniCard>

              <MiniCard title="Stack">
                <div className="flex flex-wrap gap-2">
                  {(p.stack ?? []).slice(0, 6).map((x) => <Tag key={x}>{x}</Tag>)}
                </div>
              </MiniCard>

              <MiniCard title="Evaluation mindset">
                {(p.evaluation ?? []).length ? (
                  <ul className="list-disc space-y-1 pl-5">
                    {(p.evaluation ?? []).slice(0, 2).map((x) => <li key={x}>{x}</li>)}
                  </ul>
                ) : (
                  <span className="text-slate-600">Add split/backtest + key metric.</span>
                )}
              </MiniCard>
            </div>
            ...
            {p.metrics ? (
              <div className="mt-3 grid gap-3 sm:grid-cols-3">
                {p.metrics.model ? <MiniCard title="Model">{p.metrics.model}</MiniCard> : null}
                {p.metrics.metric ? <MiniCard title="Key metric">{p.metrics.metric}</MiniCard> : null}
                {p.metrics.scale ? <MiniCard title="Scale">{p.metrics.scale}</MiniCard> : null}
                {p.metrics.latencyOrCost ? <MiniCard title="Runtime / cost">{p.metrics.latencyOrCost}</MiniCard> : null}
                {p.metrics.monitoring ? <MiniCard title="Monitoring">{p.metrics.monitoring}</MiniCard> : null}
              </div>
            ) : null}
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="text-sm text-slate-700">
                Want the 2-minute version of this project + what I’d improve next?
              </div>
              <div className="flex flex-wrap gap-3">
